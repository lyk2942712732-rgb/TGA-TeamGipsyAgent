...
| 5 | Kali 容器能执行真实命令 | 阻塞 | 镜像**已可构建**（见下方「镜像构建链路的修复」），但尚未发布：仓库无 `sandbox-v*` tag，发布 workflow 从未触发，GHCR 探测 `team-gipsy/tga-kali-ctf-web` 与 `lyk2942712732-rgb/tga-kali-ctf-web` 均 HTTP 403（对照 `homebrew/core/git` HTTP 200）。22 个 profile 的 image 仍为 `REPLACE_WITH_RELEASE_DIGEST`。**解除条件已从「修好构建」收窄为「推一个 tag」** |
...
- `config/sandbox.json` 中 22 个 profile 的 image 均为 `...@sha256:REPLACE_WITH_RELEASE_DIGEST`。
  （`docker_sandbox.template` 已修复为真实 digest。）
...
- 该 workflow 发布到 `ghcr.io/${{ github.repository_owner }}`，与 `sandbox.json` 中写死的 `ghcr.io/team-gipsy/...` 命名空间不一致，发布后仍需修正镜像前缀。
  `scripts/resolve_sandbox_digests.py` 已将 registry 参数化，可消除该假设。

**此前还叠加了一层阻塞——镜像根本无法构建**。该层已解除：`sandbox-runtime / images`
现已通过，22 个镜像均可构建并通过契约校验与 CRITICAL 漏洞扫描。

解除剩余阻塞的条件：推送 `sandbox-v*` tag 触发发布 → workflow 现已包含回写步骤，
自动将真实 digest 写回 `config/sandbox.json` → 重跑 `deploy/wsl-rootfs/provision.sh`
→ `tga up` 即由 `degraded` 转为 `ready`。
