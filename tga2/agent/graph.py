...
class TaskGraph:
    def __init__(self, dependencies: RuntimeDeps) -> None:
        ...
        graph.add_edge("load_task", "supervisor")
    ...
    @staticmethod
    def _route_review(state: TGAState) -> str:
        if not state.get("review_passed") and state.get("iteration", 0) < state.get(
            "max_iterations", 2
        ):
            return "retry"
        index = state.get("intent_index", 0)
        if state.get("review_passed") and index + 1 < len(state.get("intent_ids", [])):
            return "advance"
        return "report"
