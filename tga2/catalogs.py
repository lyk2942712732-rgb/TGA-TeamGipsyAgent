...
def host_capabilities() -> list[dict[str, Any]]:
    definitions = [
        ...
        (
            "read_skill",
            "Read one Skill Markdown document",
            "knowledge",
            "passive",
            {
                "name": {"type": "string"},
...
def kali_profiles(settings: KaliSandboxSettings) -> list[dict[str, Any]]:
    value = {
        ...
        "allowed_executables": [],
        "session_executables": [],
        "network_mode": "per-task ExecutionPolicy",
        "input_mount": "./inputs (task-scoped copy)",
        "scratch_mount": ". and /tmp (writable)",
        "shared_artifact_mount": "Runtime capture (not directly mounted)",
        "limits": {
