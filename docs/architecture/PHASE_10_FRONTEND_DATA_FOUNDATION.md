...
loads a normalized `RuntimeStore`; the previous `SessionRuntimePage` remains
available by setting `VITE_RUNTIME_PAGE=legacy`.

## Boundary and compatibility

`features/runtime/models/normalize.ts` is the only schema discriminator. It
parses schema-v6 task snapshots into the current store and maps schema-v5
snapshots into the same shape through an explicit read-only legacy adapter.
React components do not branch on schema versions. The old page receives a
separate compatibility view from `legacy-view.ts`, so its temporary rollback
path does not shape the new store.
...
Unknown events remain in the timeline without changing domain state. Schema-v5
events go through the dedicated legacy lifecycle reducer.
