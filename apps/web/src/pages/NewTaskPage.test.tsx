...
describe("NewTaskPage multimodal input flow", () => {
  ...
  it("shows one prompt composer for text and attachments in step two", async () => {
    const user = userEvent.setup();
    render(<NewTaskPage onCreated={vi.fn()} />);
    await user.click(screen.getByRole("button", { name: /输入与资源/ }));
    expect(screen.getByText(/第二步：输入与资源/)).toBeInTheDocument();
  ...
  it("keeps step four limited to execution boundaries", async () => {
    const user = userEvent.setup();
    render(<NewTaskPage onCreated={vi.fn()} />);
    await user.click(screen.getByRole("button", { name: /团队与策略/ }));
  ...
  it("summarizes only globally available MCP services and submits asset ids", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /启动前检查/ }));
