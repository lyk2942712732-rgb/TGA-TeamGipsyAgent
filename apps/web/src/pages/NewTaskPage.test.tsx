...
const mocks = vi.hoisted(() => ({
  ...
  fetchAgentModelOptions: vi.fn(async (mode: string) => ({ mode, agents: [{ id: "supervisor", model: { provider_name: "OpenAI", model_name: "gpt-test" } }], models: [] })),
...
describe("NewTaskPage", () => {
  ...
  it("opens as a single modal with eight fixed scenes and no architecture/model selectors", async () => {
  ...
  it("submits the description as initial prompt and includes multimodal files", async () => {
    ...
    await waitFor(() => expect(mocks.createTask).toHaveBeenCalledWith(expect.objectContaining({
      name: "Web challenge", mode: "penetration_test", goal: "分析目标并拿到 flag", input: { text: "", fileIds: ["asset-challenge.png"] },
    })));
