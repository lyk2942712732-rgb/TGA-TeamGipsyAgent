...
describe("NewTaskPage multimodal input flow", () => {
  ...
  it("shows the fixed blackboard architecture instead of an architecture selector", async () => {
    const user = userEvent.setup();
    await renderPage();
    await user.click(screen.getByRole("button", { name: /黑板协作架构/ }));
    expect(screen.getByText("PostgreSQL 黑板")).toBeInTheDocument();
    expect(screen.getByText("两个 Worker 容器")).toBeInTheDocument();
    expect(screen.queryByLabelText("执行策略")).toBeNull();
  });
  ...
  it("summarizes the fixed worker tools and submits asset ids", async () => {
    const user = userEvent.setup();
    ...
    expect(await screen.findByText(/Shell \/ Bash、文件读写/)).toBeInTheDocument();
    expect(await screen.findByText(/2 个可供 Agent 按名称检索和读取/)).toBeInTheDocument();
    expect(await screen.findByTestId("preflight-passed")).toHaveTextContent("全部检查通过");
    expect(screen.queryByText(/自动可用 MCP/)).toBeNull();
  ...
  it("reads scene choices from the backend scene contract", async () => {
    const user = userEvent.setup();
    await renderPage();
    expect(screen.getByRole("group", { name: /题目类型：场景/ })).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /应急响应/ }));
    expect(screen.getByRole("button", { name: /应急响应/ })).toHaveAttribute("aria-pressed", "true");
  });
});
