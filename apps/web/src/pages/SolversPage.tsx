...
export function SolversPage() {
  ...
  const availableModels = useMemo(() => (models?.providers ?? []).flatMap((provider) => provider.models.map((model) => ({ provider, model }))), [models]);
  ...
  return <div className="ref-page solvers-page">
    ...
    <div className="solver-settings-layout ref-fill">
      ...
      {selected ? <main className="ref-card solver-settings-detail">
        ...
        {tab === "model" ? <section className="solver-detail-section"><h3><Cpu size={17} />默认模型绑定</h3><label className="wide">供应商 / 模型<select value={`${selected.provider_id}::${selected.model_id}`} onChange={(event) => { const [provider_id, model_id] = event.target.value.split("::"); patch({ provider_id, model_id }); }}>{availableModels.map(({ provider, model }) => { const compatible = modelCompatible(selected.runtime, provider.protocol); return <option key={`${provider.id}::${model.id}`} value={`${provider.id}::${model.id}`} disabled={!compatible}>{provider.name} / {model.name}{compatible ? "" : "（SDK 协议不兼容）"}</option>; })}</select></label><p className="field-help">这里设置新任务的默认模型。下拉框展示 Models 页面读取的全部模型；与当前 Agent SDK 协议不兼容的项目会标明且不可选择。</p></section> : null}
