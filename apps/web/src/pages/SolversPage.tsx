  ...
  return <div className="ref-page solvers-page">
    ...
    <div className="solver-settings-layout ref-fill">
      ...
      {selected ? <main className="ref-card solver-settings-detail">
        ...
        {tab === "model" ? <section className="solver-detail-section"><h3><Cpu size={17} />默认模型绑定</h3><label className="wide">供应商 / 模型 / 协议<select value={`${selected.provider_id}::${selected.model_id}::${selected.protocol}`} onChange={(event) => { const [provider_id, model_id, protocol] = event.target.value.split("::"); patch({ provider_id, model_id, protocol: protocol as typeof selected.protocol }); }}>{availableModels.map(({ provider, model, protocol }) => { const compatible = modelCompatible(selected.runtime, protocol); return <option key={`${provider.id}::${model.id}::${protocol}`} value={`${provider.id}::${model.id}::${protocol}`} disabled={!compatible}>{provider.name} / {model.name} / {protocolName(protocol)}{compatible ? "" : "（SDK 协议不兼容）"}</option>; })}</select></label><p className="field-help">同一供应商和模型可按 Agent SDK 选择不同兼容协议；后端会据此拼接实际运行端点。</p></section> : null}
