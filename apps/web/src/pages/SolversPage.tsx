...
function SolverConfigPanel({ agentId, agent, definition, availableModels, busy, onPatch, onSave, onClose }: { agentId: string; agent: AgentConfig; definition: AgentDefinition | null; availableModels: Array<{ provider: NonNullable<ModelsConfig["providers"]>[number]; model: NonNullable<ModelsConfig["providers"]>[number]["models"][number]; protocol: NonNullable<ModelsConfig["providers"]>[number]["protocols"][number] }>; busy: boolean; onPatch: (value: Partial<AgentConfig>) => void; onSave: () => void; onClose: () => void }) {
  const compatibleModels = availableModels.filter(({ protocol }) => modelCompatible(agent.runtime, protocol));
  return <aside className="ref-card solver-config-drawer" aria-label={`${agent.display_name} Solver 配置`}>
    ...
    <div className="solver-config-scroll">
      ...
      <section className="solver-detail-section"><header><span>02</span><h3><Cpu size={17} />模型</h3></header><label className="wide">供应商 / 模型 / 协议<select value={`${agent.provider_id}::${agent.model_id}::${agent.protocol}`} disabled={!compatibleModels.length} onChange={(event) => { const [provider_id, model_id, protocol] = event.target.value.split("::"); onPatch({ provider_id, model_id, protocol: protocol as AgentConfig["protocol"] }); }}>{compatibleModels.length ? compatibleModels.map(({ provider, model, protocol }) => <option key={`${provider.id}::${model.id}::${protocol}`} value={`${provider.id}::${model.id}::${protocol}`}>{provider.name} / {model.name} / {protocolName(protocol)}</option>) : <option value="">没有可用的兼容模型</option>}</select></label><p className="field-help">仅显示当前 Agent SDK 可直接使用的模型协议；后端会据此拼接实际运行端点。</p></section>
