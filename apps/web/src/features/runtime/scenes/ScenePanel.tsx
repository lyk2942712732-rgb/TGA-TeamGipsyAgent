...
function sceneProjection(store: RuntimeStore): { title: string; items: Array<[string, string | number]> } {
  ...
  switch (store.task.mode) {
    ...
    case "reverse_analysis": return { title: "逆向分析视图", items: [["Binary Metadata", `${artifacts.length} artifacts`], ["Function / Call Graph", String(raw.call_graph_summary ?? "未投影")], ["Strings / Config", String(raw.strings_summary ?? "未投影")], ["Dynamic Execution", String(raw.dynamic_execution ?? "未投影")], ["Recovered Logic / Protocol", `${Object.keys(store.findingsById).length} findings`]] };
