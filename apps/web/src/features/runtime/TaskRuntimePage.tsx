...
export function TaskRuntimePage({ taskId, mode = "runtime" }: { taskId: string; mode?: "runtime" | "replay" }) {
  ...
  return <section className="task-runtime-page">
    ...
    {store.legacy ? <div className="runtime-legacy-banner" role="status">v5 只读回放：数据已通过单一 Legacy Adapter 投影，所有 v6 控制均已禁用。</div> : null}
    ...
    <div className="task-runtime-layout">
      ...
      <main><TaskWorkspaceTabs store={viewStore} tab={selection.tab} selectedSolverId={selectedSolverId} selectedIntentId={selection.intentId} readonly={mode === "replay" || store.legacy} onChanged={refresh} onTab={(tab: RuntimeTab) => setSelection({ tab })} onSolver={(solverId) => setSelection({ solverId })} onIntent={(intentId) => setSelection({ intentId, solverId: viewStore.intentsById[intentId]?.assignedSolverId ?? selection.solverId })} /></main>
    ...
    <InterventionDialog store={store} open={interventionOpen && mode === "runtime" && !store.legacy} onClose={() => setInterventionOpen(false)} onSubmitted={refresh} />
  </section>;
}
