...
function solver(value: unknown): RuntimeSolver { const item = record(value); return { taskId: string(item.task_id), solverId: string(item.solver_id), definitionId: string(item.definition_id), orchestrationRole: string(item.orchestration_role), specialties: strings(item.specialties), parentSolverId: nullableString(item.parent_solver_id), assignedIntentId: nullableString(item.assigned_intent_id), status: string(item.status), currentSummary: string(item.current_summary), modelSnapshot: record(item.model_snapshot), capabilityBinding: record(item.capability_binding), budgetUsage: budget(item.budget_usage), timestamps: timestamps(item.timestamps) }; }
function intent(value: unknown): RuntimeIntent { const item = record(value); return { taskId: string(item.task_id), intentId: string(item.intent_id), kind: string(item.kind), title: string(item.title), objective: string(item.objective), status: string(item.status), assignedSolverId: nullableString(item.assigned_solver_id), dependencies: strings(item.dependencies), priority: number(item.priority), successCriteria: strings(item.success_criteria), expectedEvidence: strings(item.expected_evidence), stopConditions: strings(item.stop_conditions), allowedTools: strings(item.allowed_tools), budget: budget(item.budget), createdAt: string(item.created_at), updatedAt: string(item.updated_at) }; }
function workerResult(value: unknown): RuntimeWorkerResult { const item = record(value); return { resultId: string(item.result_id), solverId: string(item.solver_id), intentId: string(item.intent_id), status: string(item.status), summary: string(item.summary), artifactIds: strings(item.artifact_ids), evidenceClaimIds: strings(item.evidence_claim_ids), knowledgeIds: strings(item.knowledge_ids), findingIds: strings(item.finding_ids), limitations: strings(item.limitations), completionStatus: string(item.completion_status, "incomplete"), criterionAssessments: array(item.criterion_assessments).map(record), budgetUsage: budget(item.budget_usage) }; }
