...
function solver(id: string, role: string, status: string, parent: string | null, assigned: string | null, specialties: string[], usage: Record<string, number>) {
  return { task_id: "task", solver_id: id, definition_id: `${role}-v1`, orchestration_role: role, specialties, parent_solver_id: parent, assigned_intent_id: assigned, status, current_summary: `${id} current activity`, model_snapshot: { model: "test-model" }, skill_snapshot: { count: 2, names: [`${role}-method`, "task-common"], selector: "v1", total_chars: 100 }, tool_policy: { allowed_capabilities: ["http.request", "workspace.read"], allowed_tool_groups: ["resource"], content_sha256: "hash" }, budget_usage: usage, timestamps: { started_at: "2026-07-30T00:00:00Z" } };
}
...
function runtimeEvent(seq: number, type: string, solverId: string | null, intentId: string | null, payload: Record<string, unknown>) { return { schema_version: 6, id: `event-${seq}`, task_id: "task", seq, type, solver_id: solverId, intent_id: intentId, payload: { schema_version: 1, ...payload }, created_at: `2026-07-30T00:00:0${seq}Z` }; }
