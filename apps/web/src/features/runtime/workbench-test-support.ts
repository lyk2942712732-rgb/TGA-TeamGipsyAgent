...
function intent(id: string, title: string, status: string, assigned: string | null, dependencies: string[]) { return { task_id: "task", intent_id: id, kind: "investigate", title, objective: title, status, assigned_solver_id: assigned, dependencies, priority: 1, success_criteria: [`Verify ${title}`], expected_evidence: ["Artifact-backed observation"], stop_conditions: ["Stop when the criterion is evidenced"], allowed_tools: ["run_command"], budget: {}, created_at: "", updated_at: "" }; }
