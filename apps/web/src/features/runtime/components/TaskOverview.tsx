...
import { selectActiveSolvers, selectConfirmedFindings, selectPendingApprovals } from "../models/selectors";
...
export function TaskOverview({ store, onSelectSolver, onSelectIntent }: { store: RuntimeStore; onSelectSolver: (id: string) => void; onSelectIntent: (id: string) => void }) {
  ...
  return <div className="task-overview">
    ...
    <section><header className="runtime-section-title"><div><span>CONFIRMED FINDINGS</span><h3>已确认发现</h3></div><small>{confirmed.length}</small></header>{confirmed.length ? <ul>{confirmed.map((item) => <li key={item.findingId}>{item.title}</li>)}</ul> : <p className="runtime-empty">暂无已确认发现</p>}</section>
    <section className="overview-risks"><header className="runtime-section-title"><div><span>BLOCKERS</span><h3>阻塞与审批</h3></div><small>{blocked.length}</small></header>{blocked.map((solver) => <p key={solver.solverId}><b>{solver.solverId}</b>：{solver.status}</p>)}{!blocked.length ? <p className="runtime-empty">当前没有阻塞</p> : null}</section>
