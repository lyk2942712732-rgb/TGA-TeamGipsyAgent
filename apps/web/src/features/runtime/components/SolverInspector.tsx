...
function Config({ solver }: { solver: RuntimeSolver }) { return <section><h4>冻结配置</h4><dl className="solver-summary-list"><Item label="Definition" value={solver.definitionId} /><Item label="Model" value={summary(solver.modelSnapshot)} /><Item label="Skills" value={summary(solver.skillSnapshot)} /><Item label="Tool Policy" value={summary(solver.toolPolicySummary)} /><Item label="创建时间" value={String(solver.timestamps.createdAt ?? "未投影")} /><Item label="更新时间" value={String(solver.timestamps.updatedAt ?? "未投影")} /></dl></section>; }
...
function SkillRecord({ value, index }: { value: unknown; index: number }) { const item = record(value); const reasons = list(item.selection_reasons).map(String).join("、"); return <article><b>{String(item.name ?? `Skill ${index + 1}`)}</b><small>{[item.version ? `v${String(item.version)}` : "", item.selection_reason ?? item.reason ?? reasons].filter(Boolean).join(" · ")}</small></article>; }
function summary(value: Record<string, unknown>): string { return Object.entries(value).slice(0, 8).map(([key, item]) => `${key}: ${Array.isArray(item) ? item.join("、") : String(item)}`).join(" · ") || "未投影"; }
