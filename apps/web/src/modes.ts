export const TASK_MODES = ["ctf", "penetration_test", "incident_response", "vulnerability_research", "reverse_analysis"] as const;
...
export const MODE_PROFILES: Record<TaskMode, { label: string; description: string; defaultGoal: string }> = {
  ...
  reverse_analysis: { label: "逆向分析", description: "分析二进制、固件、字节码或恶意样本，恢复所需逻辑、行为、配置或数据。", defaultGoal: "逆向分析目标并以真实分析产物支撑所需的逻辑、行为或数据恢复结论。" },
};
