...
export type ModeProfileContract = {
  ...
  allowed_input_kinds: string[];
  required_conditions: string[];
  recommended_capabilities: string[];
  completion_validator: string;
  report_sections: string[];
  uses_flag: boolean;
  fields: Array<{
    key: string;
    label: string;
    type: "text" | "textarea" | "select" | "number" | "checkbox" | "csv";
    options?: string[];
    min?: number;
    max?: number;
  }>;
  advanced_settings?: string[];
  mode_config_schema?: Record<string, unknown>;
  execution_policy_schema?: Record<string, unknown>;
};
...
export type TaskListItem = {
  ...
  solver_total?: number;
  highest_severity?: "info" | "low" | "medium" | "high" | "critical" | null;
};
...
export async function createTask(
  ...
  form.set(
    "prompt",
    [request.goal, request.input.text].filter(Boolean).join("\n\n"),
  );
  form.set("scene_id", request.mode);
  form.set("agent_models", JSON.stringify(request.agentModels ?? {}));
  for (const id of request.input.fileIds) {
    const file = pendingFiles.get(id);
    if (file) form.append("files", file, file.name);
  }
  const value = await requestJson<{ id: string; state: string }>(
    "/api/v3/tasks",
  ...
  return {
    ...
    status: value.state,
    scheduled: true,
    mcp_capabilities: { server_ids: [], tools: [] },
  };
}
...
export async function preflightTask(
  ...
  if (unavailable.length)
    throw new Error(
      `以下 Agent 的模型或密钥未配置：${unavailable.map((agent) => agent.id).join("、")}`,
    );
  return {
    fingerprint: `${request.id}:${request.mode}`,
    task_id: request.id,
    checks: [
      {
        id: "config",
        status: "passed",
        detail: "场景、Agent 模型与输入已就绪",
      },
    ],
    skill_catalog: {
      strategy: "worker_on_demand",
    ...
    mcp_catalog_version: "tga3",
    model_verification_id: "config",
  };
}

export async function fetchModeProfiles(): Promise<{
  schema_version: number;
  profiles: ModeProfileContract[];
}> {
  ...
  return {
    ...
    profiles: value.scenes.map((scene) => ({
      id: scene.id,
      label: scene.name,
      description: scene.description,
      default_goal: scene.system_prompt.split("。")[0] || scene.description,
      default_mode_config: { mode: scene.id },
      default_execution_policy: structuredClone(policy),
      allowed_input_kinds: ["text", "file", "image"],
      required_conditions: [],
      recommended_capabilities: [],
      completion_validator: "final_candidate",
      report_sections: ["思路", "Finding", "最终结果"],
      uses_flag: true,
      fields: [],
    })),
  };
  ...
  return {
    ...
    mediaKind: file.type.startsWith("image/")
      ...
      : file.type.startsWith("text/")
        ...
        : "other",
    size: file.size,
    sha256: "pending-task-create",
    status: "uploaded",
  };
}

export async function deleteStagedInput(assetId: string) {
  pendingFiles.delete(assetId);
...
export async function deleteTask(taskId: string) {
  await requestJson<void>(`/api/v3/tasks/${encodeURIComponent(taskId)}/stop`, {
    method: "POST",
  });
  return { task_id: taskId, deleted: true };
}
export const getLLMSettings = () =>
  requestJson<LLMSettings>("/api/v2/settings/llm");
export const updateLLMSettings = (payload: LLMSettingsUpdate) =>
  requestJson<LLMSettings>("/api/v2/settings/llm", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
export const verifyLLMSettings = () =>
  requestJson<{
    configured: boolean;
    reachable: boolean;
  ...
  }>("/api/v2/settings/llm/verify", { method: "POST" });
...
function providerView(
  provider: ModelsDocument["providers"][number],
): ModelProvider {
  return {
    id: provider.id,
    name: provider.name,
...
export async function createModelProvider(payload: {
  ...
  if (value.providers.some((item) => item.id === id))
  ...
  const provider: ModelsDocument["providers"][number] = {
    ...
    api_keys: [
      {
        id: keyId,
        label: payload.api_key_label ?? "Primary",
...
export async function discoverProviderModels(
  providerId: string,
): Promise<{ provider_id: string; count: number; models: ProviderModel[] }> {
  return requestJson(
    `/api/v3/config/models/${encodeURIComponent(providerId)}/discover`,
    { method: "POST" },
  );
}
export async function updateProviderEndpoint(
  ...
  const value = await loadModels();
  const provider = value.providers.find((item) => item.id === providerId);
  ...
  return {
    provider: providerView(
      refreshed.providers.find((item) => item.id === providerId)!,
    ),
  };
}
export async function addProviderAPIKey(
  providerId: string,
  payload: { api_key: string; label?: string },
): Promise<{ api_key: ProviderAPIKey }> {
  const value = await loadModels();
  const provider = value.providers.find((item) => item.id === providerId);
  if (!provider) throw new Error("供应商不存在");
  const key = {
  ...
  await discoverProviderModels(providerId);
  const refreshed = await loadModels();
  const projected = providerView(
    refreshed.providers.find((item) => item.id === providerId)!,
  ).api_keys;
  return { api_key: projected.find((item) => item.id === key.id)! };
}
export async function selectProviderAPIKey(
  ...
  const value = await loadModels();
  const provider = value.providers.find((item) => item.id === providerId);
  if (!provider || !provider.api_keys.some((key) => key.id === keyId))
...
export async function verifyProviderModel(providerId: string, modelId: string) {
  const value = await loadModels();
  const provider = value.providers.find((item) => item.id === providerId);
...
export type SkillDetail = SkillSetting & {
  instructions: string;
  documents: SkillDocument[];
};
export type SkillDocumentDetail = SkillDocument & { content: string };
export type ModePromptSettings = {
  id: TaskMode;
  label: string;
  methodology: string[];
  completion_focus: string;
  observer_focus: string;
};
export type AgentPromptSettings = {
  schema_version: 1;
  common_system_prompt: string;
  modes: ModePromptSettings[];
};
...
const skillMeta = (
  ...
  entrypoint: "SKILL.md",
  file_count: 1,
  total_bytes: new TextEncoder().encode(content).length,
  content_sha256: "managed-by-tga3",
  enabled: true,
});
const skillDetail = (
  name: string,
  ...
  ...skillMeta(name, description, content),
  ...
  documents: [
    {
      ...
      sha256: "managed-by-tga3",
    },
  ],
});
export async function fetchSkillSettings(): Promise<{
  schema_version: number;
  root: string;
  skills: SkillSetting[];
}> {
  const rows = await requestJson<SkillIndexItem[]>("/api/v3/skills");
  return {
    schema_version: 1,
    root: "config/skills",
    skills: rows.map((item) => skillMeta(item.name, item.description)),
  };
}
export async function fetchSkillDetail(
  name: string,
): Promise<{ skill: SkillDetail }> {
  const value = await requestJson<{ name: string; content: string }>(
    skillUrl(name),
  );
  return { skill: skillDetail(name, value.content) };
}
export async function createSkill(payload: {
  ...
  await requestJson(skillUrl(payload.name), {
    method: "PUT",
    headers: jsonHeaders,
    body: JSON.stringify({ content }),
  });
  return { skill: skillDetail(payload.name, content, payload.description) };
}
export async function importSkill(file: File): Promise<{ skill: SkillDetail }> {
  void file;
  throw new Error(
    "TGA3 当前按 Skill 目录中的 SKILL.md 管理，请使用“新建 Skill 包”后粘贴内容。",
  );
}
export async function updateSkill(
  name: string,
  payload: Pick<SkillDetail, "tags" | "version" | "instructions"> & {
    description: string;
  },
): Promise<{ skill: SkillDetail }> {
  await requestJson(skillUrl(name), {
    method: "PUT",
    headers: jsonHeaders,
    body: JSON.stringify({ content: payload.instructions }),
  });
  return {
    skill: skillDetail(name, payload.instructions, payload.description),
  };
}
export async function putSkillDocument(
  name: string,
  payload: { path: string; content: string },
): Promise<{ skill: SkillDetail }> {
  if (payload.path !== "SKILL.md")
    throw new Error("TGA3 当前每个 Skill 只维护 SKILL.md");
  await requestJson(skillUrl(name), {
    method: "PUT",
    headers: jsonHeaders,
    body: JSON.stringify({ content: payload.content }),
  });
  return { skill: skillDetail(name, payload.content) };
}
export async function fetchSkillDocument(
  name: string,
  path: string,
): Promise<{ document: SkillDocumentDetail }> {
  if (path !== "SKILL.md") throw new Error("文档不存在");
  const value = await requestJson<{ content: string }>(skillUrl(name));
  return {
    document: {
      path,
      title: path,
      size: new TextEncoder().encode(value.content).length,
      sha256: "managed-by-tga3",
      content: value.content,
    },
  };
}
export async function deleteSkillDocument(name: string, path: string) {
  if (path === "SKILL.md") throw new Error("SKILL.md 是必需入口，不能单独删除");
  return { name, path, deleted: false };
}
export async function deleteSkill(name: string) {
  await requestJson<void>(skillUrl(name), { method: "DELETE" });
  return { name, deleted: true };
}
export const fetchAgentPromptSettings = () =>
  requestJson<AgentPromptSettings>("/api/v2/settings/agent-prompts");
export const updateAgentPromptSettings = (payload: AgentPromptSettings) =>
  requestJson<AgentPromptSettings>("/api/v2/settings/agent-prompts", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
