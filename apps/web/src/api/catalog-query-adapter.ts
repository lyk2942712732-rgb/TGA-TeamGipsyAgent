...
export async function fetchSystemHealth(): Promise<SystemHealthResult> {
  ...
  const [process, models, skills] = await Promise.all([
    requestJson<HealthResponse>("/api/v3/health"),
  ...
  return {
    components: [
      component("runtime", "TGA3 Control Plane", process.status === "ok" ? "healthy" : "degraded", process.service, latencyMs),
      component("models", "Model Providers", models.providers.length ? "available" : "unavailable", `${models.providers.length} 个供应商 · ${Object.keys(models.bindings).length} 个 Agent 绑定`, null),
      component("skills", "Skills", "available", `${skills.length} 个按需读取 Skill`, null),
      component("workers", "Worker Containers", "available", "容器由任务生命周期按需启动，实时状态请在任务运行页查看", null),
    ],
  };
}
