...
export const tga3Api = {
  ...
  config: () => request<ConfigBundle>("/config"),
  saveConfig: (config: ConfigBundle) => request<ConfigBundle>("/config", json("PUT", config)),
  ...
  saveSkill: (name: string, content: string) => request<SkillDocument>(
